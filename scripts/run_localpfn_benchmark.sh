...
tabzilla_root="${1:?Usage: scripts/run_localpfn_benchmark.sh TABZILLA_DATASET_ROOT}"
...
for method in ${methods}; do
  if [[ "${method}" == "full" ]]; then
    "${python_command}" scripts/run_benchmark.py \
      --benchmark localpfn --tabzilla-root "${tabzilla_root}" --method full \
  ...
  if [[ "${method}" == "random" && -n "${random_ratios}" ]]; then
    for ratio in ${random_ratios}; do
      "${python_command}" scripts/run_benchmark.py \
        ...
        --method random --random-ratio "${ratio}" --device "${device}" --seed "${seed}" \
  ...
  for k in ${k_values}; do
    "${python_command}" scripts/run_benchmark.py \
      --benchmark localpfn --tabzilla-root "${tabzilla_root}" \
      --method "${method}" --k "${k}" --device "${device}" --seed "${seed}" \
      --evaluation-split "${evaluation_split}" --output "${output}" --resume \
      "${extra_args[@]}"
  done
done
